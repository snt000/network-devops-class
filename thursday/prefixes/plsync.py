...
with open("./pl.yml") as fP:
    prefixLists = yaml.load_all(fP.read())

# Load the Jinja2-template
with open("./pltemplate.j2") as fT:
    cfgTemplate = fT.read()
...
for router_name, host_ip in routers.items():
    ...
    dev = Device(host=host_ip, user="lab", password="lab123", gather_facts=False)
    dev.open()
    ...
    if dev.cu.diff() is None:
        ...
        askForCommit = raw_input("Commit changes? (Yes, No)")
        if askForCommit in ["Yes","ggg"]:
